...
class Indexer:
    ...
    def list_categories(self):
        ...
        if self.username == "" or self.passkey == "":
            return xbmc.executebuiltin(
                "Addon.OpenSettings(%s)" % xbmcaddon.Addon().getAddonInfo("id")
            )
    ...
    def addCategoryItem(self, category, labelValue, actionValue):
        ...
        list_item.setArt({"thumb": categoryThumb, "icon": categoryThumb})
    ...
    def list_torrents(self, categoryId, startIndex):
        ...
        if not startIndex:
            ...
            torrents = self.FLTorrentProvider.getLatestTorrentsByCategoryID(
                category["id"]
            )
        else:
    ...
    def show_torrents(self, categoryId, torrents, startIndex, paged=True):
        ...
        for index in range(startIndex, endIndex):
            ...
            if self.metaDataAvailable:
                if "imdb" in torrent:
                    ...
                    metadata = self.movieInfoProvider.getMovieInfo(
                        imdbID=torrent["imdb"]
                    )
                    if metadata:
                        if "poster_path" in metadata:
                            posterFullPath = self.movieInfoProvider.getPosterFullPath(
                        ...
                        if "backdrop_path" in metadata:
                            fanartFullPath = self.movieInfoProvider.getBackdropFullPath(
            ...
            xbmcplugin.addDirectoryItem(
                self._handle, url, list_item, is_folder, totalItems=totalItems
            )
        ...
        if addNextPageItem:
            ...
            url = self.get_url(
                action="listing", startIndex=endIndex, categoryId=categoryId
            )
            is_folder = True
            xbmcplugin.addDirectoryItem(self._handle, url, list_item, is_folder)
        # Finish creating a virtual folder.
    ...
    def play_video(self, torrentId):
        ...
        torrentPathUrl = six.moves.urllib.parse.quote_plus(torrentPath)
    ...
    def startSearch(self, categoryId=None):
        ...
        if kb.isConfirmed():
            text = kb.getText()
            filtered_text = six.moves.urllib.parse.quote_plus(text)
            torrents = self.FLTorrentProvider.searchTorrents(
                "name", filtered_text, categoryId
            )
            if len(torrents) > 0:
